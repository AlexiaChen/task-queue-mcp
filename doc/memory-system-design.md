...
**v1 为什么不做（历史决策）**:
v1 时认为需要 NLP 实体抽取，纯 Go 约束下质量难保证。但实际使用中发现：
AI Agent 本身就是 LLM，天然具备实体识别能力，可以直接提供结构化三元组，无需独立 NLP 管道。

**v2 设计关键洞察（为什么可以做了）**:

| 洞察 | 说明 |
|------|------|
| **Agent 即 NLP** | 处理 issue 的 Agent 本身是 LLM，可直接输出 `(subject, predicate, object)` 三元组 |
| **replace_existing 旗标** | 单值谓词（status, assigned_to）自动失效旧事实；多值谓词（has_label, depends_on）追加共存 |
| **闭开区间 `[valid_from, valid_to)`** | 防止边界双重匹配，SQL: `valid_from <= t AND (valid_to IS NULL OR valid_to > t)` |
| **事务性替换** | auto-invalidation + 新三元组插入在同一 DB 事务内完成 |

详细设计见 **第 7 节：时序知识图谱设计**。
...
| 版本 | 特性 | 状态 |
|------|------|------|
| v1.0 | 基础记忆系统（FTS5 BM25 搜索、去重、分类） | ✅ 已实现 (Issue #35) |
| v1.3 | 时序知识图谱（三元组、时间窗口、auto-invalidation） | ✅ 已实现 (Issue #38) |
| v2.0 | 向量搜索（如 Go 生态出现纯 Go vector lib） | 🔮 等待 CGO_ENABLED=0 兼容的向量库 |
| v2.1 | 查询意图分类（借鉴 gbrain 的零成本模式匹配） | 🔮 搜索使用数据积累后考虑 |
| v2.2 | 跨项目记忆（全局搜索 + 项目过滤） | 🔮 多项目使用场景验证后考虑 |
...
传统 KG 需要独立 NLP 管道做实体抽取。本系统的关键洞察是：
**AI Agent 本身就是 LLM，天然具备实体识别能力**。Agent 在处理 issue 过程中，
可以直接提供结构化的 `(subject, predicate, object)` 三元组，无需任何额外 NLP 依赖。

这使得 KG 从一个复杂的 NLP 子系统变成了一个简洁的结构化存储层。
...
type Triple struct {
    ID             int
    ProjectID      int
    Subject        string     // 实体主体 (max 1024 chars)
    Predicate      string     // 关系/属性 (max 1024 chars)
    Object         string     // 值/目标实体 (max 1024 chars)
    ValidFrom      time.Time  // 事实生效时间
    ValidTo        *time.Time // 事实失效时间 (nil = 仍然有效)
    Confidence     float64    // 置信度 0.0-1.0
    SourceMemoryID *int       // 可选：关联的原始记忆
    CreatedAt      time.Time
}
...
type StoreTripleInput struct {
    ...
    Confidence      *float64
    SourceMemoryID  *int
    ReplaceExisting bool  // 单值谓词 auto-invalidation
}
```

### 7.3 时间语义

**闭开区间 `[valid_from, valid_to)`**:
- `valid_from`：事实开始为真的时间点（包含）
- `valid_to`：事实停止为真的时间点（不包含），NULL 表示仍然有效
- 防止边界双重匹配：在时间点 T，旧事实 `valid_to=T` 已失效，新事实 `valid_from=T` 刚生效

**时间点查询 SQL**:
```sql
WHERE valid_from <= :at_time AND (valid_to IS NULL OR valid_to > :at_time)
```

### 7.4 Auto-Invalidation（自动失效）

通过 `replace_existing` 旗标控制：

| 场景 | replace_existing | 行为 |
|------|-----------------|------|
| 单值谓词（status, assigned_to, uses） | `true` | 找到同 subject+predicate 的活跃三元组 → 设其 `valid_to = new.valid_from` → 插入新三元组 |
| 多值谓词（depends_on, has_label） | `false` | 直接插入，不影响已有三元组 |

**事务保证**: auto-invalidation（UPDATE）+ 新三元组插入（INSERT）在同一 DB 事务内完成。
失败时两个操作都回滚，不会产生孤立状态。

**Backdated writes**: 当新三元组的 `valid_from` 早于当前时间时，旧三元组的 `valid_to`
设为 `new.valid_from`（不是 `now()`），保持时间线的逻辑一致性。
...
-- 索引
CREATE INDEX idx_triples_project ON memory_triples(project_id);
CREATE INDEX idx_triples_subject ON memory_triples(project_id, subject);
CREATE INDEX idx_triples_subj_pred ON memory_triples(project_id, subject, predicate);
CREATE INDEX idx_triples_valid_to ON memory_triples(project_id, valid_to);
```

### 7.6 TripleStorage 接口

```go
type TripleStorage interface {
    StoreTriple(ctx context.Context, triple *Triple) error
    GetTriple(ctx context.Context, projectID, id int) (*Triple, error)
    QueryTriples(ctx context.Context, projectID int, filter TripleFilter) ([]*Triple, error)
    InvalidateTriple(ctx context.Context, projectID, id int, validTo time.Time) error
    DeleteTriple(ctx context.Context, projectID, id int) error
    DeleteTriplesByProject(ctx context.Context, projectID int) error
}
```
...
| 工具 | 模式 | 描述 |
|------|------|------|
| `triple_store` | readonly | 存储三元组，支持 `replace_existing` 自动失效 |
| `triple_query` | readonly | 按 subject/predicate 查询，支持 `active_only` 和 `at_time` |
| `triple_invalidate` | admin | 手动使三元组失效（设置 valid_to） |
| `triple_delete` | admin | 物理删除三元组 |

### 7.8 REST API

| 方法 | 路径 | 描述 |
|------|------|------|
| POST | `/api/projects/{id}/triples` | 创建三元组 |
| GET | `/api/projects/{id}/triples/{tripleId}` | 获取单个三元组 |
| GET | `/api/projects/{id}/triples` | 查询三元组（支持 ?subject, ?predicate, ?active_only, ?at_time） |
| PATCH | `/api/projects/{id}/triples/{tripleId}/invalidate` | 使三元组失效 |
| DELETE | `/api/projects/{id}/triples/{tripleId}` | 删除三元组 |

### 7.9 Agent 工作流集成

KG 在 Agent 处理 issue 循环中的角色：

```
Pre-flight (Step 3a Part 3):
  triple_query(subject=<entity>, active_only=true)
  → 获取实体的当前关系（依赖、状态、技术栈）

Compound (Step 5c-iii):
  triple_store(subject, predicate, object, replace_existing)
  → 记录 issue 中产生的实体关系（新组件、依赖变化、状态更新）
```

**典型三元组模式**:
- `("memory_system", "uses", "FTS5_BM25")` — 技术选型
- `("memory_system", "status", "implemented")` — 组件状态
- `("triple_store", "depends_on", "memory_system")` — 依赖关系
- `("issue_38", "assigned_to", "agent")` — 任务分配
